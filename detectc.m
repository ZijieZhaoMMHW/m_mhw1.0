function [MHW,mclim,m90,mhw_ts,category_ts]=detectc(temp,time,cli_start,cli_end,mhw_start,mhw_end,varargin)
    ...
    case 'MHW'
        ...
        for i=1:x_size
            for j=1:y_size
                ...
                mhw_ts(i,j,isnan(squeeze(temp_mhw(i,j,:))))=nan;
                
                if sum(isnan(squeeze(temp_mhw(i,j,:))))~=size(temp_mhw,3)
                    
                    maysum=zeros(size(temp_mhw,3),1);
                    
                    maysum(squeeze(temp_mhw(i,j,:))>=squeeze(m90(i,j,indextocal)))=1;
                    ...
                    if ~isempty(potential_event)
                        ...
                        if ~isempty(potential_event)
                            ...
                            for le=1:size(potential_event,1)
                                ...
                                mrow=squeeze(temp_mhw(i,j,event_here(1):event_here(2)));
                                manom=mrow-mcl;
                                mca=mth-mcl;
                            ...
                            MHW{i,j}=[mhwstart mhwend mduration mhwint_max mhwint_mean mhwint_var mhwint_cum repmat(i,size(mhwstart,1),1) repmat(j,size(mhwstart,1),1) mhwcategory];
                        end
                    end
                end
            end
