...
%  temperature (TEMP, starting at DATA_START), climatology (MCLIM) 
...
%   MHW/MCS in TEMP.
%
%   data_start - Start date, in datenum format, of TEMP. 
%   [updated M Chandler, May 2021]
%
%   date_start - Start date on the figure in datenum format.
%   [updated M Chandler, May 2021]
%   
%   date_end - End date on the figure in datenum format. 
%   [updated M Chandler, May 2021]
%   
...
period_plot=data_start:1:(data_start+size(temp,3)-1);
...
switch vEvent
    case 'MHW'
        ...
        for i=1:size(MHW,1)
            ...
            y1=m90_plot(x1-data_start+1);
            
            x2=x1(end:-1:1);
            y2=(temp_here(x2-data_start+1));
    ...
    case 'MCS'
        ...
        for i=1:size(MHW,1)
            MHW_here=period_mhw(i,:);
            x1=(MHW_here(1):MHW_here(2))';
            y1=temp_here(x1-data_start+1);
            
            x2=x1(end:-1:1);
            y2=(m90_plot(x2-data_start+1));
